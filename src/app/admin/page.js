...
export default function AdminLoginPage() {
  ...
  useEffect(() => {
    document.title = 'Admin Login — T2Hub';
    try {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (json.success) {
        ...
        if (role !== 'master_agent' && role !== 'admin') {
          setError('Access denied. Admin or Master Agent role required.');
          setLoading(false);
          return;
        }
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className={`rounded-3xl border border-white/10 bg-white/[0.05] p-8 shadow-2xl shadow-black/40 backdrop-blur-xl sm:p-10 ${shake ? 'animate-[shake_0.4s_ease]' : ''}`}>
          <div className="mb-8 flex flex-col items-center text-center">
            ...
            <p className="mt-1.5 text-sm text-slate-500">T2Hub Exam Center Console</p>
          </div>
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <div className="relative">
                ...
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Enter admin email" autoComplete="username" required className="w-full rounded-xl border border-white/10 bg-white/[0.05] py-3 pl-11 pr-4 text-sm text-white transition-all placeholder:text-slate-500 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/50" />
              </div>
            </div>
            <div>
              ...
              <div className="relative">
                ...
                <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter password" autoComplete="current-password" required className="w-full rounded-xl border border-white/10 bg-white/[0.05] py-3 pl-11 pr-11 text-sm text-white transition-all placeholder:text-slate-500 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/50" />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 transition-colors hover:text-slate-200" aria-label={showPassword ? 'Hide password' : 'Show password'}>
                  {showPassword ? (
            ...
            <button type="submit" disabled={loading} className="group relative w-full overflow-hidden rounded-xl bg-gradient-to-r from-indigo-500 to-blue-600 py-3.5 text-sm font-semibold text-white shadow-lg shadow-indigo-500/25 transition-all hover:from-indigo-400 hover:to-blue-500 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-70">
              {loading ? <span className="flex items-center justify-center gap-2"><div className="h-4 w-4 rounded-full border-2 border-white/30 border-t-white animate-spin" />Signing in...</span> : 'Sign In'}
            </button>
          </form>
          <p className="mt-6 text-center text-[11px] text-slate-600">T2Hub v0.2.0 — RBAC Enabled</p>
        </div>
      </div>
    </div>
  );
